...
internal static class UpdateChecker
{
    ...
    // Derived from <Version> in the csproj rather than hardcoded, so the
    // version shown in the tray, the version compared against GitHub, and
    // the version stamped on the exe can never drift apart.
    public static string CurrentVersion
    {
        get
        {
            var v = Application.ProductVersion;
            int plus = v.IndexOf('+'); // .NET appends +<git-hash> to the informational version
